...
# 	cv2.destroyAllWindows()
	...
	patternCenter=tuple((cX,cY))
 	# centroid = cv2.circle(origImg,patternCenter,radius = 15, color = (0,255,0),thickness = -1) # Thickness -1 fills circle
 	# cv2.namedWindow('cent',cv2.WINDOW_NORMAL)
 	# cv2.imshow('cent',centroid)
 	# cv2.resizeWindow('cent',600,600)
 	# cv2.waitKey(0)
 	# cv2.destroyAllWindows()
	...
	for i in range(0,len(contours)):
		...
		if (ratio > 90) and (ratio<115):
			cornerContours.append(i) # Appends index of desired contours
	...
	for i in cornerContours[::2]:
		...
		for k in range(0,4):
			x+=approx[k][0][0]
			y+=approx[k][0][1]
		centers.append(tuple((x/4,y/4))) ## WHY DO WE DIVIDE BY 4 here?
		squares.append(approx)

	if len(squares)<3:
		return returnArray,0

	#Find the relative orientation of the corner squares (cardinal, not pose)
