...
import glob
import math
import QRPoints_debug as QRPoints
...
def draw(img,corners,imgpts):
        ...
        """
        corner=tuple(int(corners[0].ravel()[0]),int(corners[0].ravel()[1]))
...
# for fname in [images[0]]:
        ...
        img = image
        corners = imagePoints
        imgpts = projectedPoints
        
        # Determine origin ('corner') and draw all three axes
        # corner=tuple(corners[0].ravel()) # This line didn't work on Surface python, see adjusted:
        ...
        #Get camera position
        camPos=-np.matrix(rotationMat).T * np.matrix(tvecs) ## Don't use @ since using old np.matrix type
